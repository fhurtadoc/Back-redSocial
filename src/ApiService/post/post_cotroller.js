        ...
        Post_dao.create_coment(coment, (res, err)=>{
            if(res){                
                ...
                Post_dao.coment_asocs(asoc_coment, (res_asoc, err)=>{
                    if(res_asoc) return res.send({menssaje:"Creado Correctamente", codigo:200})
                    if(err) return res.send({menssaje:"error en query al asociar comentario", codigo: 402});
                })
            }
            if(err){
                if(err) return res.send({menssaje:"error en query al crear comentario", codigo: 402});
            }
        })
    },
